...
console.log(usePage().props.products.data)
const brands = usePage().props.brands;
const categories = usePage().props.categories;

// Add computed property to safely access products data
    ...
    <section class="  p-3 sm:p-5">
        ...
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="(product, index) in productsList" :key="product.id"
                                ...
                                <th scope="row"
                                    class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white max-w-[200px] truncate"
                                    :title="product.title">
                                    {{ product.title }}
                                </th>
                ...
                <nav v-if="products && products.links"
                    class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                    aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">{{ products.from || 0 }}-{{
                            products.to || 0 }}</span>
                        of
                        <span class="font-semibold text-gray-900 dark:text-white">{{ products.total || 0 }}</span>
                    </span>
                    <ul class="inline-flex items-stretch -space-x-px">
                        <li>
                            <a href="#" @click.prevent="products.prev_page_url && router.get(products.prev_page_url)"
                        ...
                        <li v-for="link in products.links.slice(1, -1)" :key="link.label">
                            <a href="#" @click.prevent="link.url && router.get(link.url)" :class="{
                                'z-10 text-primary-600 bg-primary-50 border-primary-300': link.active,
                                'text-gray-500 bg-white border-gray-300': !link.active
                            }"
                                class="flex items-center justify-center text-sm py-2 px-3 leading-tight border hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                {{ link.label }}
                            </a>
                        </li>

                        <li>
                            <a href="#" @click.prevent="products.next_page_url && router.get(products.next_page_url)"
