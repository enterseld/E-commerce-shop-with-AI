      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="(item, i) in forecastData" :key="i"
                  :class="item.isForecast ? 'bg-green-50' : 'hover:bg-gray-50'">
                  <td class="p-3 border-t">
                    {{ item.month }}
                    <span v-if="item.isForecast" class="ml-2 text-xs text-green-600 font-semibold">(прогноз)</span>
                  </td>
                  ...
                  <td class="p-3 border-t text-right">
                    <span v-if="item.actual" :class="Math.abs(item.actual - item.predicted) / item.actual < 0.1
                      ? 'text-green-600'
                      : 'text-orange-600'">
        ...
        <div class="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <ul class="text-sm text-blue-800 space-y-1">
            <li><strong>R²:</strong> показує, наскільки добре модель пояснює варіацію даних (0–1, краще ближче до 1)
            </li>
...
// Дані замовлень
const rawOrders = usePage().props.orders;
...
const aggregateByMonth = (orders) => {
  ...
  orders.forEach((o) => {
    ...
    monthMap[key].total += parseFloat(o.total_price || 0);
    monthMap[key].count++;
  });
